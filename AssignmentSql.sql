...
INSERT INTO employees(desgination,salarly,location) VALUE ("Software Developer Itern",15000,"Indore"),
...
UPDATE  employee_info e SET city="Shahdol" WHERE e.ID=1;

DELETE FROM employee_info WHERE city="Delhi";

UPDATE employees e SET salarly=16000 WHERE location="Indore";
UPDATE employees SET salarly=45000 WHERE empID=3;
SELECT COUNT(empID), location
FROM employees
GROUP BY location;
 
 SELECT AVG(salarly) FROM employees e WHERE e.location="Pune";
 SELECT SUM(salarly) FROM employees e WHERE e.location="Pune";
 
 SELECT * FROM employees e,
 employee_info ei WHERE e.empID=ei.empID
 ORDER BY e.empID; 
